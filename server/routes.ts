...
`;
  ...
  app.post("/api/auth/request-password-reset", authRateLimit, async (req, res) => {
    try {
      ...
      // Check if user has local auth account (not OAuth only)
      const authAccount = await storage.getAuthAccountByUserId(user.id);
      if (!authAccount || authAccount.authType !== 'local') {
        return res.json({ 
          success: true,
          message: "If an account exists with that email, a password reset link has been sent." 
        });
      ...
      await storage.setPasswordResetToken(user.id, hashedToken, expiry);
  ...
  // Verify reset token - checks if token is valid without using it
  ...
  app.post("/api/auth/reset-password", authRateLimit, async (req, res) => {
    try {
      ...
      const passwordHash = crypto
        .pbkdf2Sync(newPassword, passwordSalt, 100000, 64, 'sha256')
        .toString('hex');

      // Update password in auth_accounts table
      await storage.updateAuthAccountPassword(user.id, passwordHash, passwordSalt);
